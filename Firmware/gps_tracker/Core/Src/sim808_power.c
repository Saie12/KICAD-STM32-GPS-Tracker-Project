...
static const uint32_t g_watchdog_timeout_ms = 10000U;
...
#if SIM808_USE_MOCK
    // Simulate power mode transitions
    switch (mode) {
    case SIM808_POWER_ACTIVE:
        sim808_mock_send_command("AT+CFUN=1");
        break;
    case SIM808_POWER_IDLE:
        sim808_mock_send_command("AT+CSCLK=0");
        break;
    case SIM808_POWER_SLEEP:
        sim808_mock_send_command("AT+CSCLK=1");
        break;
    case SIM808_POWER_STANDBY:
        sim808_mock_send_command("AT+CFUN=0");
        break;
    default:
        return SIM808_ERR_HW;
    }
...
sim808_result_t sim808_power_get_battery(sim808_battery_t *bat)
{
    ...
    if (HAL_GetTick() - last_update >= 5000U) {
        last_update = HAL_GetTick();

        // Slowly decrease voltage
...
#if SIM808_USE_MOCK
    if (enable) {
        sim808_mock_send_command("AT+IWDG=1,10");
    } else {
        sim808_mock_send_command("AT+IWDG=0");
    }
#endif
